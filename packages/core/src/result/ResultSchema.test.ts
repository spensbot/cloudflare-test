...
import z from "zod"
import { OkSchema, ErrSchema, ResultSchema } from "./ResultSchema"

describe("OkSchema", () => {
  it("should validate an ok result with correct data", () => {
    const schema = OkSchema(z.string())
    const result = schema.safeParse({ ok: true, val: "test" })

    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data).toEqual({ ok: true, val: "test" })
    }
  })

  it("should reject when ok is false", () => {
    const schema = OkSchema(z.string())
    const result = schema.safeParse({ ok: false, val: "test" })

    expect(result.success).toBe(false)
  })

  it("should reject when val has wrong type", () => {
    const schema = OkSchema(z.string())
    const result = schema.safeParse({ ok: true, val: 123 })

    expect(result.success).toBe(false)
  })

  it("should work with complex data schemas", () => {
    const schema = OkSchema(z.object({
      id: z.number(),
      name: z.string(),
    }))
    ...
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data.val).toEqual({ id: 1, name: "Alice" })
    }
  })

  it("should work with array schemas", () => {
    const schema = OkSchema(z.array(z.number()))
    const result = schema.safeParse({ ok: true, val: [1, 2, 3] })

    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data.val).toEqual([1, 2, 3])
    }
  })
})

describe("ErrSchema", () => {
  it("should validate an error result with correct error", () => {
    const schema = ErrSchema(z.string())
    const result = schema.safeParse({ ok: false, err: "error message" })

    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data).toEqual({ ok: false, err: "error message" })
    }
  })

  it("should reject when ok is true", () => {
    const schema = ErrSchema(z.string())
    const result = schema.safeParse({ ok: true, err: "error message" })

    expect(result.success).toBe(false)
  })

  it("should reject when err has wrong type", () => {
    const schema = ErrSchema(z.string())
    const result = schema.safeParse({ ok: false, err: 123 })

    expect(result.success).toBe(false)
  })

  it("should work with complex error schemas", () => {
    const schema = ErrSchema(z.object({
      code: z.number(),
      message: z.string(),
    }))
    const result = schema.safeParse({
    ...
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data.err).toEqual({ code: 404, message: "Not found" })
    }
  })
})
